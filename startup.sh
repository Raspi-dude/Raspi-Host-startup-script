...
TOS_NOTICE="# By editing this file manually, you still agree to the Terms of Service hosted at https://host.raspidude.eu.org/tos"
...
if [[ "$tos_status" != "true" ]]; then
    ...
    echo "Visit https://host.raspidude.eu.org/tos"
...
if [ -z "${VERSION}" ] || [ "${VERSION}" = "latest" ]; then
    ...
    DOWNLOAD_URL=$(curl -s "https://api.github.com/repos/BeamMP/BeamMP-Server/releases" \
        | grep -A20 "\"tag_name\": \"${VERSION}\"" \
        | grep "browser_download_url" \
        | grep "$MATCH" \
        | head -n 1 \
        | cut -d '"' -f 4)
...
if [ -z "$DOWNLOAD_URL" ]; then
    echo "Error: Could not find download URL. Please report this to Raspi_dude."
    exit 1
fi
